...
void reportFatalError(char* what) {
	gameApi.reportFatalError(gameApi.ctx, what);
}
...
void reportNonFatalError(char* what) {
	gameApi.reportNonFatalError(gameApi.ctx, what);
}
...
void pluginOnLoad(tf_v0_GameApi newApi) {
	gameApi = newApi;
	gameApi.log(gameApi.ctx, "tf2-mc-export pluginOnLoad running");

	gameApi.registerDebugButton(gameApi.ctx, "tf2-mc-export debugButton", &debugButtonCallback, 0);
}
...
TF_V0_DEFINE_INITIALISE_PLUGIN_MODULE_FUNC {
	...
	pluginsApi.registerPlugin(pluginsApi.ctx, thisPlugin);
}
